...
$sql2 = "select id,firstname,middlename,lastname,schooldesc,coursedesc,documenttype,paymentid,preparedby,reviewedby,applicationtype,status from tbl_process a inner join tbl_schools b inner join tbl_courses c where a.schoolid=b.schoolid and a.courseid=c.courseid and a.active='yes' and status='rejected' and a.schoolid=$schoolID";
                ...
                <tbody>
                    <?php while($fetch2=mysqli_fetch_assoc($result2)) { ?>
                        <tr>
                            ...
                            <td><?=$fetch2['status']?></td>
                            <td><a href="viewRejectedDetails.php?id=<?=$fetch2['id']?>" class="btn btn-primary">View</a></td>
                        </tr>
                    <?php } ?>
                </tbody>
